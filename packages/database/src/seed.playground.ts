...
import {
    NOTIFICATION_SCOPE,
    NotificationScope,
} from "@trydarwin/types/notifications/notification-scope";
...
const ORG = {
    slug: "appx",
    name: "Appx",
    description: "Ships Nocturn, a scheduling product that runs on darwin for its own bug queue.",
};

const PROJECT = {
    slug: "nocturn",
    name: "Nocturn",
    summary: "Scheduling product — the repo darwin's runners clone, build and open PRs against.",
    description:
                ...
                data: {
                    ...
                    contextBlobUrl:
                        plan.status === WorkerStatus.Dead
                            ? null
                            : `https://artifacts.trydarwin.dev/workers/${plan.sandboxId ?? "pending"}/context.jsonl`,
    ...
    for (let index = 0; index < specs.length; index++) {
        ...
        const issue = await prisma.issue.create({
            data: {
                ...
                prUrl,
                prBranch: hasPr ? `darwin/issue-${index + 1}` : null,
                specialization,
        ...
        const agentRow = (
            ...
            activityRows.push({
                issueId: issue.id,
                type,
                payload: { ...payload, actor: { name: "darwin", image: null } },
            ...
            for (let attempt = 1; attempt <= attempts; attempt++) {
                ...
                const session = await prisma.agentSession.create({
                    data: {
                        ...
                        traceUrl: `https://artifacts.trydarwin.dev/sessions/${issue.id}-${attempt}/trace.jsonl`,
    ...
    const setupSession = await prisma.setupSession.create({
        data: {
            ...
            questions: {
                create: [
                    {
                        type: AgentQuestionType.NeedSecret,
                        key: "DATABASE_URL",
                    ...
                    {
                        type: AgentQuestionType.NeedChoice,
                        key: "START_COMMAND",
                    ...
                    {
                        type: AgentQuestionType.DefineSuccess,
                        key: "VERIFICATION",
                    ...
                    {
                        type: AgentQuestionType.ApproveCost,
                        key: "SANDBOX_BUDGET",
                    ...
                    {
                        type: AgentQuestionType.NeedAccess,
        ...
        JSON.stringify(
            {
                ...
                setupQuestions: await prisma.agentQuestion.count({
                    where: { setupSessionId: setupSession.id },
                }),
