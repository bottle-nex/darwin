...
export default async function merge_review_controller(req: Request, res: Response) {
    try {
        ...
        if (!token) {
            ResponseWriter.custom(
                ...
                false,
                "GITHUB_NOT_LINKED",
                "Connect your GitHub account to merge this pull request.",
                409,
            );
            return;
        }
        ...
        try {
            await GithubPullsService.mergePullRequest(token, resolved.ref);
        } catch (error) {
            ResponseWriter.custom(
                res,
                false,
                "MERGE_FAILED",
                github_error_message(error, "Could not merge this pull request."),
                422,
            );
            return;
        }
        ...
        ResponseWriter.success(res, header, "Pull request merged");
    } catch (error) {
        console.error("error in merge_review_controller:", error);
