        ...
        <SettingsHeroCard>
                    ...
                    {GLOWS.map(({ rgb, label, selected }) => (
                        <div
                            ...
                            className={cn(
                                ...
                                selected ? "border-primary/60" : "border-overlay/10",
                            )}
                            style={{
                                background: `linear-gradient(135deg, rgba(${rgb}, 0.3), transparent 75%), var(--color-charcoal)`,
                            }}
                        >
                            <HeroLine className="w-12 bg-overlay/25" />
                            <div className="mt-3 flex items-center justify-between gap-2">
                                <span className="text-[12px] text-overlay/70">{label}</span>
            ...
            <HeroPanel className="-top-3 left-60 w-76 rotate-2">
                <div className="flex items-center justify-between gap-3">
                    <span className="text-[13px] text-overlay/80">Background lighting</span>
                    <HeroSwitch on />
                </div>
                <div className="mt-3.5">
                    <HeroLabel>Direction</HeroLabel>
                    <div className="relative mt-2.5 h-1 rounded-full bg-overlay/10">
                        <span className="absolute left-0 h-full w-3/5 rounded-full bg-overlay/30" />
                        <span className="absolute top-1/2 left-3/5 size-3 -translate-y-1/2 rounded-full bg-overlay shadow" />
                    </div>
                </div>
            </HeroPanel>
            <HeroPanel className="top-16 -right-10 w-68 -rotate-2">
                <div className="flex items-center justify-between gap-3">
                    <span className="text-[13px] text-overlay/70">Default home view</span>
