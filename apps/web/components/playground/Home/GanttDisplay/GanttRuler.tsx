...
export default function GanttRuler() {
    const ticks = Array.from({ length: GanttTimeline.DAY_MINUTES / 5 + 1 }, (_, i) => i * 5);

    return (
        <div
            className="relative shrink-0 border-b border-border"
            style={{ width: GanttTimeline.DAY_WIDTH, height: GanttTimeline.RULER_HEIGHT }}
        >
            {ticks.map((m) => {
                const isHour = m % 60 === 0;
                ...
                return (
                    ...
                    >
                        <div
                            ...
                            style={{ height }}
                        />
                        {isHour && m < GanttTimeline.DAY_MINUTES && (
