...
export function SelectionBarButton({
    ...
    return (
        <Button
            ...
            type="button"
            className={cn(
                "flex cursor-pointer items-center gap-1.25 rounded-full border border-overlay/5 bg-overlay/4 py-1.25 pr-3.5 pl-3 text-[11px] text-overlay shadow-[var(--shadow-card)] transition-colors hover:bg-overlay/6",
                className,
            )}
            {...props}
        >
    ...
    return (
        <AnimatePresence>
            {count > 0 && (
                ...
                >
                    <div className="surface-menu pointer-events-auto flex items-center gap-1 rounded-full py-1.75 pr-2 pl-5">
                        <span className="text-[13px] text-overlay/90">{count} selected</span>
                        {hint && (
